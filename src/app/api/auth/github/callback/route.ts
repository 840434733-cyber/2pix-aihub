  ...
  try {
    ...
    if (error || !code) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=${error || 'no_code'}`
      )
    }

    const clientId = process.env.GITHUB_CLIENT_ID
    const clientSecret = process.env.GITHUB_CLIENT_SECRET
    const redirectUri = `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/api/auth/github/callback`

    if (!clientId || !clientSecret) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=oauth_not_configured`
      )
    }
    ...
    if (!accessToken) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=token_exchange_failed`
      )
    }
    ...
    if (!githubUser.id) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=invalid_user`
      )
    }
    ...
    if (!user) {
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=create_user_failed`
      )
    }
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://www.2pix.cn'}/login?error=server_error&err=${encodeURIComponent(errMsg.slice(0, 200))}`
    )
  }
}
